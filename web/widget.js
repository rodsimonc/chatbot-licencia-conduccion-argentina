/* Widget de chat embebible: "Licencia conducción Argentina".
 * Uso en cualquier página:
 *   <script src="https://TU-SERVIDOR/widget.js" data-api="https://TU-SERVIDOR" defer></script>
 */
(function () {
  var script = document.currentScript;
  var API = (script && script.getAttribute('data-api')) || window.location.origin;
  var TITLE = (script && script.getAttribute('data-title')) || 'Licencia conducción Argentina';
  ...
  var btn = el('<button class="lca-btn" aria-label="Abrir chat">💬</button>');
  var panel = el(
    '<div class="lca-panel" role="dialog" aria-label="' + TITLE + '">' +
      '<div class="lca-head"><div>' + TITLE + '<small>Preguntas sobre el Manual del Conductor (ANSV)</small></div>' +
        '<button class="lca-x" aria-label="Cerrar">×</button></div>' +
      '<div class="lca-msgs"></div>' +
      '<div class="lca-input"><input type="text" placeholder="Escribí tu pregunta…" ' +
        'aria-label="Tu pregunta"><button>Enviar</button></div>' +
      '<div class="lca-foot">Respuestas basadas en el manual. Puede contener errores.</div>' +
    '</div>'
  );
  ...
  function open() {
    ...
    if (!started) { started = true; addBot('¡Hola! Soy el asistente de ' + TITLE + '. Preguntame lo que quieras sobre el Manual del Conductor: licencias, seguridad vial, documentación, señales, etc.'); }
    input.focus();
  }
  ...
  function addMsg(text, who, sources) {
    ...
    if (sources && sources.length) {
      ...
      s.textContent = '📄 Manual, página' + (sources.length > 1 ? 's ' : ' ') + sources.join(', ');
  ...
  async function send() {
    ...
    var typing = el('<div class="lca-msg bot"><div class="lca-bubble"><span class="lca-typing">Escribiendo…</span></div></div>');
    ...
    try {
      ...
      typing.remove();
      addBot(data.answer || 'No pude responder eso.', data.sources || []);
    } catch (e) {
      typing.remove();
      addBot('No me pude conectar con el servidor. Revisá que el chatbot esté corriendo.');
    } finally {
  ...
  // Permitir configurar el título desde el backend (opcional).
